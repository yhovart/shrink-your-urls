...
@Profile("jpa")
public interface UrlEntityJpARepository extends JpaRepository<UrlEntity, UUID>, UrlRepository {

  
    default <S extends UrlEntity> S saveIfCodeIsUnique(S entity) throws NonUniqueCodeException {
        try{
            return this.saveAndFlush(entity);
        }catch(DataIntegrityViolationException e){
            // TODO faire plus fiable / beau
            if (e.getCause() instanceof ConstraintViolationException ||e.getCause() instanceof jakarta.validation.ConstraintViolationException) {
